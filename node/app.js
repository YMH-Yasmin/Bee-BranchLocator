// to remove before commit: 1) API KEY, 2) outlets JSON URL.
...
// const outlets = require("./outlets(clubs).json");

const getOutlets = async () => {
    try {
        return await axios.get("https://cloudFunctionURL.json");
    } catch (error) {
...
const setOutlets = async () => {
    ...
    console.log(`Got ${Object.entries(outlets.data).length} outlet entries.`);
    console.log(`Got ${Object.keys(outlets.data).length} outlet keys.`);
};
